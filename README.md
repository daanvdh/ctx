...
**Variables in frontmatter** – `execution-session` and `output-entry` may reference ctx values of the triggering session, e.g. `execution-session: $STORY_ID`, rendered when the trigger fires. Matcher fields (`trigger-session`, `ancestor`, `entries`) stay literal, and schedule-driven triggers have no triggering session, so their `execution-session` must be literal too.
